...
cluster.
  ...
  -provision.vagrant            provision to a single-node vagrant host
...
// Provisioning flags
var fProvisionVagrant = flag.Bool("provision.vagrant", false,
...
// Nomad version flags
// TODO(tgross): right now these just override each other, local_file > sha > version,
// but we should probably assert this behavior.
...
// Environment flags
// TODO(tgross): would be nice if we could default to environment from the
// provisioner, if available
...
func (f *Framework) Run(t *testing.T) {
	info, err := f.provisioner.SetupTestRun(provisioning.SetupOptions{})
	if err != nil {
		t.Fatalf("could not provision cluster: %v", err)
	}
	defer f.provisioner.TearDownTestRun(info.ID)
...
func (f *Framework) runSuite(t *testing.T, s *TestSuite) (skip bool, err error) {
	...
	info, err := f.provisioner.SetupTestSuite(provisioning.SetupOptions{
		Name:         s.Component,
	...
	defer f.provisioner.TearDownTestSuite(info.ID)
	...
	for _, c := range s.Cases {
		...
		info, err := f.provisioner.SetupTestCase(provisioning.SetupOptions{
			Name:         name,
			ExpectConsul: s.Consul,
			ExpectVault:  s.Vault,
		})
		...
		defer f.provisioner.TearDownTestCase(info.ID)
