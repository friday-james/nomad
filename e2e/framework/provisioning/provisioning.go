...
type Provisioner interface {
	...
	SetupTestRun(opts SetupOptions) (*ClusterInfo, error)

	// SetupTestSuite is called at the start of each TestSuite.
	// TODO: no current provisioner implementation uses this, but we
	// could use it to provide each TestSuite with an entirely seperate
	// Nomad cluster.
	SetupTestSuite(opts SetupOptions) (*ClusterInfo, error)

	// SetupTestCase is called at the start of each TestCase in every TestSuite.
	SetupTestCase(opts SetupOptions) (*ClusterInfo, error)
	...
	TearDownTestCase(clusterID string) error

	// TearDownTestSuite is called after every TestSuite.
	TearDownTestSuite(clusterID string) error

	// TearDownTestRun is called at the end of the entire test run.
	TearDownTestRun(clusterID string) error
}
