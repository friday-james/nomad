      ...
      "servers" : [for server in aws_instance.server.* :
        {
          ...
          "deployment" : {
            ...
            "steps" : [
              ...
              "sudo /ops/shared/config/provision-server.sh aws ${var.server_count} '${var.nomad_sha}' 'indexed/server-${index(aws_instance.server, server)}.hcl'"
            ],
          }
        }
      ],
      "clients" : concat([for client in aws_instance.client_linux.* :
        {
          ...
          "deployment" : {
            ...
            "steps" : [
              ...
              "sudo /ops/shared/config/provision-client.sh aws '${var.nomad_sha}' 'indexed/client-${index(aws_instance.client_linux, client)}.hcl'"
            ],
          }
        }
        ],
        [for client in aws_instance.client_windows.* :
          {
            ...
            "deployment" : {
              ...
              "steps" : [
                "& C:\\ops\\shared\\config\\provision-windows-client.ps1 -Cloud aws -NomadSha ${var.nomad_sha} -Index 1"
              ]
            }
          }
      ])
